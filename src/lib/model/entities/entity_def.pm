...
sub register($) {
    ...
    ) = db::selectrow_array('select entity_type, entity_description from entity_def def join entity_type typ on typ.entity_type_id = def.entity_type_id where def.entity_name = ?', undef, $name);
